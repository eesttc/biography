...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['action_type']) && $_POST['action_type'] == 'add') {
        $sql = "INSERT INTO achievements (title, description, img1_link, img1_alt, img1_description, img2_link, img2_alt, img2_description) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $edit_id = $_POST['id'];
        $sql = "UPDATE achievements SET title=?, description=?, img1_link=?, img1_alt=?, img1_description=?, img2_link=?, img2_alt=?, img2_description=? WHERE achievement_id=?";
        $stmt = $conn->prepare($sql);
    ...
    header("location: manage_achievements.php?message=" . urlencode($message) . "&error=" . urlencode($error));
    exit;
}
...
<div class="container">
    ...
    <div class="form-section">
        <h2>Add/Modify</h2>
        <?php
        $current_achievement = ['achievement_id' => '', 'title' => '', 'description' => '', 'img1_link' => '', 'img1_alt' => '', 'img1_description' => '', 'img2_link' => '', 'img2_alt' => '', 'img2_description' => ''];
        ...
        if ($action == 'edit' && $id) {
            ...
            if ($result->num_rows > 0) {
                $current_achievement = $result->fetch_assoc();
        ...
        <form action="manage_achievements.php" method="post">
            ...
            <input type="hidden" name="id" value="<?php echo htmlspecialchars($current_achievement['achievement_id']); ?>">

            <div class="form-group">
                <label for="title">Title:</label>
                <input type="text" id="title" name="title" value="<?php echo htmlspecialchars($current_achievement['title']); ?>" required>
            </div>
            <div class="form-group">
                <label for="description">Description:</label>
                <textarea id="description" name="description" rows="5" required><?php echo htmlspecialchars($current_achievement['description']); ?></textarea>
            </div>
            <div class="form-group">
                <label for="img1_link">Link image 1:</label>
                <input type="text" id="img1_link" name="img1_link" value="<?php echo htmlspecialchars($current_achievement['img1_link']); ?>">
            </div>
            <div class="form-group">
                <label for="img1_alt">Alt image 1:</label>
                <input type="text" id="img1_alt" name="img1_alt" value="<?php echo htmlspecialchars($current_achievement['img1_alt']); ?>">
            </div>
            <div class="form-group">
                <label for="img1_description">Description image 1:</label>
                <input type="text" id="img1_description" name="img1_description" value="<?php echo htmlspecialchars($current_achievement['img1_description']); ?>">
            </div>
            <div class="form-group">
                <label for="img2_link">Link image 2:</label>
                <input type="text" id="img2_link" name="img2_link" value="<?php echo htmlspecialchars($current_achievement['img2_link']); ?>">
            </div>
            <div class="form-group">
                <label for="img2_alt">Alt image 2:</label>
                <input type="text" id="img2_alt" name="img2_alt" value="<?php echo htmlspecialchars($current_achievement['img2_alt']); ?>">
            </div>
            <div class="form-group">
                <label for="img2_description">Description image 2:</label>
                <input type="text" id="img2_description" name="img2_description" value="<?php echo htmlspecialchars($current_achievement['img2_description']); ?>">
            </div>
            <button type="submit" class="btn-submit"><?php echo ($form_action_type == 'edit' ? 'Modify' : 'Add'); ?></button>
            <a href="manage_achievements.php" class="btn-submit" style="background-color: #f0ad4e;">Cancel</a>
        </form>
    </div>
    ...
    <table>
        ...
        <tbody>
            <?php
            $sql = "SELECT * FROM achievements ORDER BY achievement_id ASC";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    echo "<tr>";
                    echo "<td>" . htmlspecialchars($row['achievement_id']) . "</td>";
                    ...
                    echo "<a href='manage_achievements.php?action=edit&id=" . $row['achievement_id'] . "' class='edit'>Modify</a>";
                    echo "<a href='manage_achievements.php?action=delete&id=" . $row['achievement_id'] . "' class='delete' onclick=\"return confirm('Are you sure to delete?');\">Delete</a>";
                    echo "</td>";
                    echo "</tr>";
                }
            } else {
