...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['action_type']) && $_POST['action_type'] == 'add') {
        $sql = "INSERT INTO ref (highlight, link, description) VALUES (?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sss", $highlight, $link, $description);
        if ($stmt->execute()) {
            $message = "Add Success!";
        } else {
        ...
        $edit_id = $_POST['id'];
        $sql = "UPDATE ref SET highlight=?, link=?, description=? WHERE reference_id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssi", $highlight, $link, $description, $edit_id);
        if ($stmt->execute()) {
    ...
    header("location: manage_ref.php?message=" . urlencode($message) . "&error=" . urlencode($error));
    exit;
}

// Xử lý yêu cầu GET (Xóa)
if ($action == 'delete' && $id) {
    $sql = "DELETE FROM ref WHERE reference_id = ?";
    $stmt = $conn->prepare($sql);
...
<div class="container">
    ...
    <div class="form-section">
        <h2>Add/Modify</h2>
        <?php
        $current_ref = ['reference_id' => '', 'highlight' => '', 'link' => '', 'description' => ''];
        $form_action_type = 'add';
        
        if ($action == 'edit' && $id) {
            $sql = "SELECT * FROM ref WHERE reference_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("i", $id);
            $stmt->execute();
            $result = $stmt->get_result();
            if ($result->num_rows > 0) {
                $current_ref = $result->fetch_assoc();
        ...
        <form action="manage_ref.php" method="post">
            <input type="hidden" name="action_type" value="<?php echo $form_action_type; ?>">
            <input type="hidden" name="id" value="<?php echo htmlspecialchars($current_ref['reference_id']); ?>">

            <div class="form-group">
                <label for="highlight">Highlight:</label>
                <input type="text" id="highlight" name="highlight" value="<?php echo htmlspecialchars($current_ref['highlight']); ?>" required>
            </div>
            <div class="form-group">
                <label for="link">Link:</label>
                <input type="text" id="link" name="link" value="<?php echo htmlspecialchars($current_ref['link']); ?>" required>
            </div>
            <div class="form-group">
                <label for="description">Description:</label>
                <textarea id="description" name="description" rows="5" required><?php echo htmlspecialchars($current_ref['description']); ?></textarea>
            </div>
            
            <button type="submit" class="btn-submit"><?php echo ($form_action_type == 'edit' ? 'Modify' : 'Add'); ?></button>
            <a href="manage_ref.php" class="btn-submit" style="background-color: #f0ad4e;">Cancel</a>
        </form>
    </div>
    ...
    <table>
        ...
        <tbody>
            <?php
            $sql = "SELECT * FROM ref ORDER BY reference_id ASC";
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<td><a href='" . htmlspecialchars($row['link']) . "' target='_blank'>" . htmlspecialchars(substr($row['link'], 0, 50)) . "...</a></td>";
                    echo "<td>" . htmlspecialchars(substr($row['description'], 0, 150)) . "...</td>";
                    echo "<td class='actions'>";
                    echo "<a href='manage_ref.php?action=edit&id=" . $row['reference_id'] . "' class='edit'>Modify</a>";
                    echo "<a href='manage_ref.php?action=delete&id=" . $row['reference_id'] . "' class='delete' onclick=\"return confirm('Are you sure to delete?');\">Delete</a>";
                    echo "</td>";
                    echo "</tr>";
                }
            } else {
